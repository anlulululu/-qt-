...
{
    ui->edtId->setText(QString("%1").arg(id,4,10,QLatin1Char('0')));//长度，10进制，填充字符
...
void CAddStuInfoDialog::on_btnConfirm_clicked()
{
    ...
    //QVariant对象是一种泛型变量，可以转换成QString、int、double等数据类型。
...
void CAddStuInfoDialog::initUI()
{
    ...
    QRegularExpression regId("^[0-9]{4}$");//正则表达式：0-9出现4次
    ui->edtId->setValidator(new QRegularExpressionValidator(regId,this));
    //名称 允许中文和字母
    QRegularExpression regName("^[a-zA-Z\u4e00-\u9fa5]{1,}$");
    ui->edtName->setValidator(new QRegularExpressionValidator(regName,this));
    //性别
    ui->cmbSex->addItem("男");
    ui->cmbSex->addItem("女");
    ...
    ui->edtPhone->setValidator(new QRegularExpressionValidator(regPhone,this));
